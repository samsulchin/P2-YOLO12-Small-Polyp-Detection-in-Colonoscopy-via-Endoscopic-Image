...
def add_image(dataset, image_path, mask_path, group_id):
    """Process 1 labeled image: extract polyps from mask, record."""
    try:
    ...
    for j, (xmin, ymin, xmax, ymax, seg, bba) in enumerate(polyps):
        ANNS.append(dict(dataset=dataset, image=image_path, group_id=group_id,
                         ...
                         seg_area=seg, bbox_area=bba,
                         size_coco=coco_size(seg), size_clinical=clinical_size(bba),
                         diam_px=diameter_px(seg)))
...
def summarize():
    ...
    print("\n" + "=" * 78)
    print("SIZE AUDIT SUMMARY (COCO threshold on segmentation area)")
    print("=" * 78)
    ...
    print("-" * len(header))
    print("img+ = positive image | img- = negative frame | group = number of group_id (anti-leakage unit)")
    print("Note: area calculated at ORIGINAL mask RESOLUTION. small<1024px, medium<9216px, large>=9216px.")
